...
export default function LandingPage() {
  ...
  return (
    ...
    <div id="view-landing" className="absolute inset-0 z-[60] flex flex-col min-h-[100dvh] bg-white custom-scroll w-full h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-start pt-32 pb-20 relative z-10 w-full max-w-5xl mx-auto px-6 lg:px-12 safe-bottom-scroll">
            
            {/* 1. IDENTITY SECTION */}
            <div className="w-full text-center flex flex-col items-center animate-fade-in-up">
                
                {/* LANDING ATOM */}
                <div className="w-40 h-40 md:w-56 md:h-56 lg:w-64 lg:h-64 mb-10 relative atom-container">
                    <div className="absolute inset-0 bg-blue-600/5 blur-[80px] rounded-full scale-125 animate-pulse-slow"></div>
                    <div className="w-full h-full atom-breathe">
                ...
                <div className="space-y-6 max-w-2xl mx-auto">
                    <p className="text-[0.65rem] md:text-xs font-bold text-slate-400 tracking-[0.3em] uppercase pl-1">NSG SYSTEM OS • Enterprise</p>
                    <h1 className="font-display text-4xl sm:text-5xl lg:text-7xl font-extrabold tracking-tight text-navy-900 leading-[1.05]">
                    ...
                    <div className="relative pl-6 border-l-4 border-blue-100 text-left mx-auto max-w-md md:max-w-lg mt-6">
                ...
                {/* CERTIFICATE */}
                <div className="mt-14 w-full max-w-lg mx-auto transform transition-all hover:scale-[1.02] duration-500">
                    <div className="bg-white/95 backdrop-blur-xl border border-slate-200/80 p-1 rounded-2xl shadow-sovereign w-full text-left">
                        <div className="bg-gradient-to-br from-white to-slate-50 rounded-xl p-5 sm:p-6 flex flex-col sm:flex-row items-center gap-6 relative overflow-hidden">
                            ...
                            <div className="relative shrink-0">
                                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-navy-900 to-blue-900 flex items-center justify-center shadow-lg shadow-navy-900/20">
                                    <ShieldCheck className="w-7 h-7 text-white stroke-[1.5]" />
                                </div>
                                <div className="absolute -bottom-1 -right-1 bg-emerald-500 text-white text-[0.4rem] font-bold px-1.5 py-0.5 rounded-full border-2 border-white flex items-center gap-0.5 shadow-sm">
                                    <Check className="w-2 h-2" /> ISO
                                </div>
                            </div>
                            <div className="text-center sm:text-left flex-1">
                                <p className="text-[0.55rem] font-bold text-emerald-600 uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 inline-block mb-1">Officially Verified</p>
                                <h3 className="text-sm text-navy-900 font-medium leading-tight mb-2">Verified by <span className="font-display font-bold text-navy-950">New Strategy Group</span></h3>
                                <div className="flex gap-6 justify-center sm:justify-start pt-3 border-t border-slate-100">
                                    <div className="text-center sm:text-left"><p className="text-lg font-display font-bold text-blue-600 leading-none">1,000+</p><p className="text-[0.5rem] font-bold text-slate-400 uppercase tracking-wide mt-0.5">Conversaciones</p></div>
                                    <div className="text-center sm:text-left border-l border-slate-100 pl-6"><p className="text-lg font-display font-bold text-indigo-600 leading-none">100+</p><p className="text-[0.5rem] font-bold text-slate-400 uppercase tracking-wide mt-0.5">Reseñas Positivas</p></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full">
                    ...
                    <button onClick={() => selectProfile('consultant')} className="group bg-white p-7 rounded-[2rem] text-left border border-slate-100 hover:border-blue-200 hover:shadow-precision transition-all duration-500 transform hover:-translate-y-1">
                        <div className="flex justify-between mb-6">
                            ...
                            <ArrowRight className="w-6 h-6 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-1" />
                        </div>
                        <h3 className="font-display font-bold text-lg text-navy-900">Consultor</h3>
                        <p className="text-xs text-slate-500 mt-1 font-medium">Gestión de Activos & Data Strategy</p>
                    </button>
                    ...
                    <button onClick={() => selectProfile('psychologist')} className="group bg-white p-7 rounded-[2rem] text-left border border-slate-100 hover:border-sky-200 hover:shadow-precision transition-all duration-500 transform hover:-translate-y-1">
                        <div className="flex justify-between mb-6">
                            ...
                            <ArrowRight className="w-6 h-6 text-slate-300 group-hover:text-sky-600 transition-transform group-hover:translate-x-1" />
                        </div>
                        <h3 className="font-display font-bold text-lg text-navy-900">Psicólogo</h3>
                        <p className="text-xs text-slate-500 mt-1 font-medium">Clínica & Análisis de Conducta</p>
                    </button>
                    ...
                    <button onClick={() => selectProfile('manager')} className="group bg-white p-7 rounded-[2rem] text-left border border-slate-100 hover:border-emerald-200 hover:shadow-precision transition-all duration-500 transform hover:-translate-y-1">
                        <div className="flex justify-between mb-6">
                            ...
                            <ArrowRight className="w-6 h-6 text-slate-300 group-hover:text-emerald-600 transition-transform group-hover:translate-x-1" />
                        </div>
                        <h3 className="font-display font-bold text-lg text-navy-900">CEO / Directivo</h3>
                        <p className="text-xs text-slate-500 mt-1 font-medium">Inteligencia de Datos & KPIs</p>
                    </button>
                    ...
                    <button onClick={() => selectProfile('patient')} className="group bg-white p-7 rounded-[2rem] text-left border border-slate-100 hover:border-teal-200 hover:shadow-precision transition-all duration-500 transform hover:-translate-y-1">
                        <div className="flex justify-between mb-6">
                            ...
                            <ArrowRight className="w-6 h-6 text-slate-300 group-hover:text-teal-600 transition-transform group-hover:translate-x-1" />
                        </div>
                        <h3 className="font-display font-bold text-lg text-navy-900">Paciente</h3>
                        <p className="text-xs text-slate-500 mt-1 font-medium">Bienestar Integral & Tracking</p>
                    </button>
                </div>
            </div>

             {/* FOOTER */}
            <div className="mt-20 w-full pt-10 pb-8 text-center flex flex-col items-center gap-6 border-t border-slate-100/50">
