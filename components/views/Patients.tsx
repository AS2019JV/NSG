    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-blue-500 transition flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto custom-scroll pb-6">
        {PATIENTS.map((p, i) => (
          <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col gap-4 hover:shadow-md transition hover:-translate-y-1 group">
            ...
            <button className="w-full py-2 bg-slate-50 text-slate-600 font-bold text-xs rounded-xl hover:bg-navy-900 hover:text-white transition">
              Ver Expediente
            </button>
          </div>
        ))}
