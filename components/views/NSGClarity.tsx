...
function StrategyCard({ strategy }: { strategy: any; }) {
  return (
    <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-500 group overflow-hidden relative border-l-4 border-l-blue-600">
      <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50/50 rounded-full blur-3xl -mr-16 -mt-16 opacity-50 group-hover:bg-blue-100/60 transition-colors"></div>
      ...
      <div className="flex items-center justify-between mb-5 relative z-10">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-bold text-navy-950 text-sm tracking-tight">Protocolo de Acción</h4>
        </div>
        <div className="flex items-center gap-2">
          <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span className="text-[0.6rem] font-black bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md uppercase tracking-tighter">
            {strategy.createdAt ? new Date(strategy.createdAt).toLocaleDateString() : 'Activo'}
          </span>
        </div>
      </div>

      <div className="mb-5 relative z-10">
        <div className="text-[9px] font-bold text-blue-600/60 uppercase tracking-[0.2em] mb-1">Meta Detectada</div>
        <p className="text-sm font-bold text-navy-900 leading-snug">{strategy.meta_detectada}</p>
      </div>

      <div className="space-y-2 relative z-10">
        {[strategy.accion_1, strategy.accion_2, strategy.accion_3].map((action, i) => (
          action && (
            <div key={i} className="flex gap-3 items-start p-3.5 bg-slate-50/50 rounded-2xl border border-slate-100 group-hover:bg-white group-hover:border-blue-100 transition-all duration-300">
              <div className="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-blue-600/10 border border-blue-600/20 flex items-center justify-center">
                <span className="text-[10px] font-black text-blue-600">{i + 1}</span>
              </div>
              <p className="text-[11px] text-slate-600 leading-relaxed font-semibold">{action}</p>
            </div>
          )
        ))}
      </div>

      <div className="mt-4 pt-4 border-t border-slate-50 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">IA Engine v4.0</span>
...
export default function NSGClarity() {
  ...
  const syncObjectives = async (isManual = false) => {
    ...
    try {
      ...
      const response = await fetch(`https://nsg-backend.onrender.com/telegram/user/${telegramId}`, { headers: { 'Authorization': jwtToken || '' } });
      if (response.ok) {
        setTelegramData(await response.json());
  ...
  const fetchStrategies = async () => {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/strategies/get?userId=${userId}`;
    const jwtToken = localStorage.getItem('token');
  ...
  useEffect(() => {
    const checkGoogle = async () => {
      const jwtToken = localStorage.getItem('token');
      try {
        const res = await fetch('https://nsg-backend.onrender.com/google/calendar/events', { headers: { 'Authorization': jwtToken || '' } });
  ...
  const handleConnect = async (p: string) => {
    ...
    if (isConnected) {
      try {
        const res = await fetch('https://nsg-backend.onrender.com/google/calendar', { method: 'DELETE', headers: { 'Authorization': jwtToken || '' } });
        if (res.ok) { setIsConnected(false); showToast('Google Calendar desconectado', 'info'); }
      } catch (e) { showToast('Error', 'error'); }
    } else {
      try {
        const res = await fetch('https://nsg-backend.onrender.com/google/auth', { headers: { 'Authorization': jwtToken || '' } });
    ...
    <div className="max-w-7xl mx-auto h-full flex flex-col animate-fade-in-up pb-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-white/50 backdrop-blur-sm p-4 rounded-2xl border border-slate-200/60 shadow-sm transition-all hover:bg-white/60">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Calendar */}
          <button onClick={() => handleConnect("Calendar")} className={clsx("flex items-center gap-3 px-4 py-2 rounded-xl border transition-all duration-300 shadow-sm active:scale-95", isConnected ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-slate-200 text-slate-600 hover:border-blue-300 hover:bg-slate-50")}>
          ...
          <div className="flex flex-col gap-6 flex-1">
            <div className="flex items-center justify-between px-2">
              <h4 className="font-bold text-navy-950 text-xl flex items-center gap-3">
                <div className="p-2.5 bg-navy-950 rounded-2xl text-white shadow-lg shadow-navy-950/20">
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-slate-100 transition active:scale-95 disabled:opacity-50"
                disabled={isLoadingStrategies}
              >
                <RefreshCw className={clsx("w-4 h-4 text-slate-400", isLoadingStrategies && "animate-spin")} />
              </button>
            </div>

            <div className="space-y-6 overflow-y-auto max-h-[700px] pr-2 custom-scroll pb-10">
              {strategies.length > 0 ? (
