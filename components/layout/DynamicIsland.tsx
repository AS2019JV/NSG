...
export default function DynamicIsland({ currentMode, setMode, selectedModel, setSelectedModel, intelligenceMode = 'pulse', setIntelligenceMode }: DynamicIslandProps) {
  ...
  return (
    <div className="relative z-50 flex flex-col items-center justify-start pt-2 xs:pt-4 gap-3 xs:gap-4 md:gap-6 w-full" ref={containerRef}>
      ...
      <div className="flex items-center gap-1.5 xs:gap-2 px-2 xs:px-3 py-1 rounded-full bg-white/50 backdrop-blur-md border border-white/60 shadow-sm animate-fade-in group cursor-default">
      ...
      <div className="relative flex items-center justify-center w-full max-w-[98vw] xs:max-w-[95vw] md:max-w-4xl mx-auto px-1 xs:px-0">
        <div className={clsx(
            "flex flex-nowrap items-center p-1 xs:p-1.5 gap-0.5 xs:gap-1",
           ...
           {allItems.map((item) => {
             ...
             return (
               <button
                 ...
                 className={clsx(
                    "flex items-center gap-1.5 xs:gap-2 px-3 xs:px-4 py-2 xs:py-2.5 rounded-full transition-all duration-300 ease-out whitespace-nowrap relative shrink-0 cursor-pointer group min-h-[44px]",
                    isActive 
               ...
               >
                 <Icon className={clsx(
                   "w-3.5 h-3.5 xs:w-4 xs:h-4 transition-colors shrink-0", 
                   isActive ? "text-blue-300" : "text-slate-500 group-hover:text-slate-300"
                 )} />
                 <span className="text-[11px] xs:text-[13px] font-medium tracking-wide">
                   {item.label}
                 </span>
               </button>
             );
