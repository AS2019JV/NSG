...
export interface ChatPayload {
    ...
    field: string; // Contexto: "NSG Copilot", "NSG Horizon"
    ...
    ({ msg, selectedModel }: { msg: Message; selectedModel: string }) => {
        ...
        if (msg.role === "assistant") {
            ...
            // 2. JSON Parsing fallback (for N8N responses)
                ...
                try {
                    ...
                    // Legacy formats
                    else if (selectedModel === "NSG AI" && parsed.nsgai)
                        displayContent = parsed.nsgai;
                    else if (
                        selectedModel === "Chat GPT" &&
                        (parsed.openai || parsed.openAI_response)
                    )
                        displayContent =
                            parsed.openai || parsed.openAI_response;
                    else if (
                        selectedModel === "Gemini" &&
                        (parsed.gemini || parsed.gemini_response)
                    )
                        displayContent =
                            parsed.gemini || parsed.gemini_response;
                    else if (
                        selectedModel === "Claude" &&
                        (parsed.claude || parsed.claude_response)
                    )
                        displayContent =
                            parsed.claude || parsed.claude_response;
        ...
        if (
            ...
            try {
                ...
                displayContent =
                    p.response || p.output || p.text || displayContent;
            } catch {}
        }
