                                    ...
                                    <ReactMarkdown 
                                        ...
                                        components={{
                                            ...
                                            div: ({node, children, ...props}) => {
                                                // Handle divs (often used by models for blocks) similar to spans for recursion
                                                return (
                                                    <div {...props}>
                                                                 ...
                                                                 return (
                                                                    <ReactMarkdown 
                                                                        ...
                                                                        components={{
                                                                            ...
                                                                            // Explicitly map inline styles to ensure they work
                                            ...
                                            span: ({node, children, ...props}) => {
                                                // Recursively parse markdown inside spans to allow styling + formatting (e.g. bold inside colored text)
                                                // We must map BLOCK elements (h1, p, etc) to SPANS to avoid invalid HTML nesting (h2 inside p/span) which causes hydration errors.
                                                return (
                                                    <span {...props}>
                                                        {React.Children.map(children, child => {
                                                            if (typeof child === 'string') {
                                                                    ...
                                                                    <ReactMarkdown 
                                                                        ...
                                                                        components={{
                                                                            ...
                                                                            blockquote: ({node, ...props}) => <span className="block border-l-[3px] border-slate-300 pl-4 py-1 my-3 text-slate-500 italic" {...props} />,
                                                                             // Explicitly map inline styles here too
                                                                            strong: ({node, ...props}) => <strong className="font-bold text-inherit" {...props} />,
                                                                            em: ({node, ...props}) => <em className="italic text-inherit" {...props} />,
                                                                            code: ({node, ...props}) => <code className="bg-slate-100 px-1.5 py-0.5 rounded text-[13px] font-mono font-medium" {...props} />
                                                                        }}
            ...
            <div className="max-w-3xl mx-auto pointer-events-auto">
                {!isContextCached ? (
                    ...
                    <form onSubmit={handleSubmit} className="relative group">
                        <div className="relative flex flex-col bg-white rounded-[28px] shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-slate-100 focus-within:shadow-[0_8px_30px_rgba(0,0,0,0.12)] focus-within:border-slate-200 transition-shadow duration-300">
                            {/* Attachment Preview */}
                            {attachment && (
