...
export interface ChatPayload {
    ...
    field: string;        // Contexto: "NSG Clarity", "NSG Horizon"
        ...
        if (msg.role === "assistant") {
            ...
            if (msg.metadata?.variations) {
                ...
                if (selectedModel === "Chat GPT" && vars["Chat GPT"]) {
                    ...
                    foundVar = true;
                } else if (selectedModel === "Gemini" && (vars["Gemini"] || vars["Gemini Pro"])) {
                    displayContent = vars["Gemini"] || vars["Gemini Pro"];
                    foundVar = true;
                } else if (selectedModel === "Claude" && vars["Claude"]) {
                    displayContent = vars["Claude"];
                    foundVar = true;
                } else if (selectedModel === "NSG AI" || selectedModel === "NSG Fusion") {
            ...
            // 2. JSON Parsing fallback (for N8N responses)
                ...
                try {
                    ...
                    // NEW N8N STRUCTURE: { answer: string | { ...models... } }
                    if (parsed.answer) {
                        if (typeof parsed.answer === "string") {
                            ...
                            if (!found) {
                                // If the answer is an object with multiple models but NOT our selected one
                                isComingSoon = true;
                                displayContent = "";
                            }
                        }
                    ...
                    // Legacy formats
                    else if (selectedModel === "NSG AI" && parsed.nsgai) displayContent = parsed.nsgai;
            ...
            // I will do it data-side in the injection to be safe and cleaner.
...
export default function ChatInterface() {
    ...
    useEffect(() => {
        ...
        const AUTHORIZED_EMAILS = ['ivanrroficial@gmail.com', 'alesuav2001@gmail.com'];
                    ...
                    messages: [
                        {
                            id: 'msg-u-1',
                            role: 'user',
                            content: "¿Qué es lo más estratégico e inteligente que puedo hacer considerando mis objetivos?",
                            type: 'text',
                            createdAt: new Date()
                        },
                        {
                            ...
                            content: `# Estrategia Maestra: Operación $1.2M (Consenso NSG)
...
| **Caballo (Duke)** | **Sistematizar.** Crear un SOP rápido o checklist y entregárselo a su equipo. | Liberar RAM mental para NSG. |
...
**Transición Duke como Caso de Éxito:** No veas la transición de Duke como una carga. Úsala estratégicamente: Documenta cómo la IA organiza su empresa y usa eso como Social Proof para vender NSG. Matas dos pájaros de un tiro: sales de la operación y creas marketing real.
...
**Profundidad sobre Amplitud:** Tu Oferta Irresistible de 1 página es la pieza de dominó que tira todas las demás. Si esa hoja no está perfecta, el CRM, los embudos y los embajadores no servirán de nada. Lo más inteligente es obsesionarte con esa hoja de papel durante tus próximas 3 sesiones de Deep Work hasta que sea imposible decirte que no."`
                ...
                useAppStore.setState(state => ({
                    chatSessions: { ...state.chatSessions, [SPECIAL_ID]: specialSession }
                }));
            }
        }
        // ---------------------------------------------
    ...
    const activeSession = currentSessionId ? chatSessions[currentSessionId] : null;
    ...
    const handleSubmit = async (e: React.SyntheticEvent) => {
        ...
        const effectiveSessionId = currentSessionId || createChatSession(currentSelectedModel, activeMode);
        ...
        try {
            ...
            const field =
                fieldMap[activeMode] || activeMode;
            ...
            const aiModel = (modelMap[selectedModel] || "claude") as 'openai' | 'claude' | 'gemini';
            ...
            if (currentSession) {
                 ...
                 updateChatSession(effectiveSessionId, { messages: newMessages });
            }
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    const formattedHistory = data.map((chat: any) => ({
                        id: chat.chatId,
                        title: chat.title,
                        updatedAt: chat.date ? new Date(chat.date).getTime() : Date.now(),
                ...
                {isSidebarOpen && (
                    ...
                    >
                         ...
                         <div className="p-4 flex items-center justify-between">
                            ...
                            <button
                                onClick={() => createChatSession(selectedModel, mode)}
                         ...
                         <div className="flex-1 overflow-y-auto px-3 custom-scroll">
                            <div className="mb-4">
                                <span className="text-xs font-semibold text-slate-500 px-3 uppercase tracking-wider">Reciente</span>
                                    ...
                                    {displaySessions.map(session => (
                                        ...
                                        >
                                            <div className="flex items-center gap-2 overflow-hidden flex-1 min-w-0">
                                                <MessageCircle className={clsx("w-4 h-4 shrink-0", currentSessionId === session.id ? "text-blue-600" : "text-slate-400")} />
                                                <span className="truncate pr-2">{session.title || "Nuevo Chat"}</span>
                                            </div>
                                        ...
                                        >
                                            <div className="flex items-center gap-3 overflow-hidden flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-col truncate">
                                                    <span className="truncate text-sm">{session.title || "Nuevo Chat"}</span>
                                                    <span className="text-[10px] text-slate-400 font-normal">
                                                        {new Date(session.updatedAt || 0).toLocaleDateString()}
                                                    </span>
                                                </div>
                                            </div>
                    ...
                    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full">
                        {isHydrating ? (
                            ...
                            <div className="flex flex-col items-center justify-center p-10 opacity-60 mt-10 text-center">
                                ...
                                <h3 className="text-lg font-semibold text-slate-700">¿En qué puedo ayudarte hoy?</h3>
                                <p className="text-sm text-slate-500 max-w-xs mt-2">NSG Intelligence puede redactar textos, analizar documentos y generar ideas creativas.</p>
                            </div>
                        ) : (
                            messages.map((msg) => (
                                <MessageItem
