...
export default function MetricsPanel({ metrics, isLoading = false }: MetricsPanelProps) {
    ...
    const productivityScore = useMemo(() => {
        ...
        const totalCompletionsWeight = Math.min((metrics.totalCompletions / (totalDays * 3)) * 100, 100) * 30; // 30% weight

        return Math.round(completionWeight + perfectDaysWeight + totalCompletionsWeight);
    }, [metrics, totalDays]);

    const getScoreColor = (score: number) => {
        if (score >= 80) return { bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-200', ring: 'ring-emerald-500' };
        if (score >= 60) return { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200', ring: 'ring-blue-500' };
        if (score >= 40) return { bg: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200', ring: 'ring-orange-500' };
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className={clsx(
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-3">
                        <div className={clsx("w-10 h-10 rounded-xl flex items-center justify-center", scoreColors.bg, scoreColors.text)}>
                            <Target className="w-5 h-5" />
                        </div>
                        <div className={clsx("px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider", scoreColors.bg, scoreColors.text)}>
                            Score
                        </div>
                    </div>
                    ...
                    <div className="flex items-baseline gap-2">
                        <span className={clsx("font-display font-bold text-4xl", scoreColors.text)}>
                            {productivityScore}
                        </span>
                    ...
                    <div className="mt-4">
                        <div className="w-full bg-white/50 rounded-full h-2 overflow-hidden">
                            <div
                                className={clsx("h-full rounded-full transition-all duration-1000", scoreColors.text.replace('text-', 'bg-'))}
            ...
            {/* Completion Rate */}
            <div className="bg-white p-6 rounded-3xl border-2 border-slate-200 hover:border-blue-300 hover:shadow-lg transition-all duration-500 relative overflow-hidden group">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-medium text-slate-600">Morning</span>
                            </div>
                            <span className="font-bold text-sm text-navy-950">{metrics.byProtocol.morning_clarity}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                                <span className="text-xs font-medium text-slate-600">Power</span>
                            </div>
                            <span className="font-bold text-sm text-navy-950">{metrics.byProtocol.power_check}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
