        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-white p-4 rounded-xl shadow-2xl border border-slate-200">
                    <p className="font-bold text-navy-950 mb-2">{payload[0].payload.date}</p>
                    <div className="space-y-1">
                        {payload.map((entry: any) => (
                            <div key={entry.name} className="flex items-center justify-between gap-4 text-sm">
                                ...
                                <span className="font-bold text-navy-950">{entry.value}</span>
                            </div>
                        ))}
    ...
    if (!chartData || chartData.length === 0) {
        return (
            <div className="bg-white p-8 sm:p-12 rounded-3xl border-2 border-dashed border-slate-200 text-center">
                ...
                <p className="text-slate-500 font-medium">No hay datos suficientes para mostrar</p>
                <p className="text-slate-400 text-sm mt-1">Completa protocolos para ver tu progreso</p>
            </div>
        );
    }
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        {period === 'week' ? 'Última semana' : 'Últimos 30 días'}
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-100">
                <div className="text-center">
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                        ...
                        <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Morning</p>
                    </div>
                    <p className="font-bold text-lg text-navy-950">
                        {chartData.reduce((sum, d) => sum + d.morning_clarity, 0)}
                    </p>
                </div>
                <div className="text-center">
                    <div className="flex items-center justify-center gap-1.5 mb-1">
                        <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                        <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Power</p>
                    </div>
                    <p className="font-bold text-lg text-navy-950">
                ...
                <div className="text-center">
                    ...
                    <p className="font-bold text-lg text-navy-950">
                        {chartData.reduce((sum, d) => sum + d.next_day_planning, 0)}
                    </p>
                </div>
            </div>
        </div>
